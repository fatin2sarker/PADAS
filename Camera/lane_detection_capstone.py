...
def draw_lines(img, lines, color=[255, 0, 0], thickness=2):
    ...
    # LINE FITTING
    if len(left_x) > 0:
        ransac_left = sk_lin_mod.RANSACRegressor(sk_lin_mod.LinearRegression(), max_trials=500, min_samples=2, residual_threshold=5,stop_n_inliers=50)
    ...
    if len(right_x) > 0:
        ransac_right = sk_lin_mod.RANSACRegressor(sk_lin_mod.LinearRegression(), max_trials=500, min_samples=2, residual_threshold=5,stop_n_inliers=50)
...
if __name__ == "__main__":
    ...
    while not exit_:
        ...
        if ret:
            ...
            processed_image = process_image(data) # ADD FILTERS ONTO CAMERA FOOTAGE (DOESNT WORK CURRENTLY)
